...
def text_collate_fn(items, context_len=49, pad_ind=200_000):
    text = torch.tensor(list(itertools.zip_longest(*items, fillvalue=pad_ind))).T
    # assert text.shape[1] <= context_len
    if text.shape[1] < context_len:
        text = torch.nn.functional.pad(text, (0, context_len - text.shape[1], 0, 0))
    else:
    ...
    def __init__(
        ...
        with jsonlines.open(metadata_file) as reader:
            if tqdm_load:
                reader = tqdm(reader)
            for obj in reader:
                if (self.images_directory / f"{obj['image']}.jpg").is_file():
